// Team data. Edit freely — the profile pages render entirely from this file.
// Benja's data is the original portfolio content; Nico / Palo / Ceci are
// starter profiles meant to be personalised.

const TEAM = [
  {
    id: "benja",
    nombre: "Benja",
    nombreCompleto: "Benja García",
    saludo: "Hola, soy Benja",
    rol: "Diseñador / Ilustrador y Programador",
    tagline: "Diseño & Front-end",
    accent: "#f1c8c8",
    portrait: "imgs/benja-portrait.png",
    email: "nagmengarcia@gmail.com",
    social: {
      behance: "#",
      linkedin: "#",
      dribbble: "#",
      mail: "mailto:nagmengarcia@gmail.com",
    },
    bio: [
      "Tengo 7 años en el área de Diseño Digital, gráfico y de interfaces de usuario. Durante los últimos 2 años me aventuré en el mundo del desarrollo front-end para complementar mi entendimiento en el desarrollo de websites.",
      "Me encanta crear sistemas escalables y automatizar / crear flujos óptimos de trabajo.",
      "Si te interesa charlar escribíme a nagmengarcia@gmail.com.",
    ],
    trayectoria: [
      { puesto: "Diseñador de Producción Sr, WPP Production.", periodo: "(2026 - HOY)" },
      { puesto: "Diseñador de Producción Ssr, WPP Production.", periodo: "(2022 - 2025)" },
      { puesto: "Diseñador Digital Ssr, Aivo.", periodo: "(2020 - 2021)" },
      { puesto: "Freelancer.", periodo: "(2019 - 2021)" },
      { puesto: "Consultor Creativo, Woola Brands.", periodo: "(2019 - 2021)" },
      { puesto: "Líder de Diseño, Keenvil.", periodo: "(2019)" },
      { puesto: "Diseñador UI Jr, Keenvil.", periodo: "(2019)" },
      { puesto: "Diseñador, Gráfica Espacio Uno.", periodo: "(2018 - 2019)" },
    ],
    estudios: {
      izquierda: [
        { texto: "Bachiller en Cs Sociales y Humanidades.", periodo: "(P. 2011)" },
        { texto: "Técnicatura en Diseño Gráfico.", periodo: "(2018)" },
        { texto: "Carrera de Desarrollador Front-end React.", periodo: "(2024)" },
      ],
      derecha: [
        { texto: "React JS, CoderHouse.", periodo: "(2024)" },
        { texto: "Javascript, CoderHouse.", periodo: "(2024)" },
        { texto: "Desarrollo Web, CoderHouse.", periodo: "(2023)" },
        { texto: "UX/UI, CoderHouse.", periodo: "(2021)" },
        { texto: "Diseño Web & JS, Nakama Workshops.", periodo: "(2021)" },
        { texto: "Tecnicatura en Diseño Gráfico.", periodo: "(2018)" },
      ],
    },
    herramientas: [
      "Illustrator", "Photoshop", "InDesign", "After Effects", "Premiere",
      "Adobe XD", "Sketch", "Figma", "Zeplin", "HTML5", "CSS & Sass",
      "React JS", "Javascript",
    ],
    idiomas: ["Español nativo", "Inglés intermedio"],
  },

  {
    id: "nico",
    nombre: "Nico",
    nombreCompleto: "Nico",
    saludo: "Hola, soy Nico",
    rol: "Editor / Postproductor Audiovisual",
    tagline: "Edición & Motion",
    accent: "#bcd4f1",
    portrait: "imgs/nico-portrait.svg",
    email: "hola@boleadora.studio",
    social: {
      behance: "#",
      linkedin: "#",
      dribbble: "#",
      mail: "mailto:hola@boleadora.studio",
    },
    bio: [
      "Editor y postproductor audiovisual con foco en montaje, ritmo y narrativa. Trabajo piezas para campañas, redes y contenido de marca de principio a fin.",
      "Me muevo cómodo entre la edición, el motion graphics y la corrección de color, cuidando que cada pieza tenga una identidad propia.",
      "¿Tenés un proyecto en mente? Escribíme a hola@boleadora.studio.",
    ],
    trayectoria: [
      { puesto: "Editor / Postproductor Sr, Boleadora Intergaláctica.", periodo: "(2023 - HOY)" },
      { puesto: "Editor de Contenido, Freelance.", periodo: "(2020 - 2023)" },
      { puesto: "Motion Designer Ssr, Estudio Audiovisual.", periodo: "(2019 - 2021)" },
      { puesto: "Asistente de Edición, Productora.", periodo: "(2018 - 2019)" },
    ],
    estudios: {
      izquierda: [
        { texto: "Realización Integral de Cine y TV.", periodo: "(2018)" },
        { texto: "Edición y Montaje Audiovisual.", periodo: "(2019)" },
      ],
      derecha: [
        { texto: "Color Grading en DaVinci Resolve.", periodo: "(2022)" },
        { texto: "Motion Graphics, After Effects.", periodo: "(2021)" },
        { texto: "Narrativa Audiovisual, Taller.", periodo: "(2020)" },
      ],
    },
    herramientas: [
      "Premiere", "After Effects", "DaVinci Resolve", "Photoshop",
      "Audition", "Media Encoder",
    ],
    idiomas: ["Español nativo", "Inglés intermedio"],
  },

  {
    id: "palo",
    nombre: "Palo",
    nombreCompleto: "Palo",
    saludo: "Hola, soy Palo",
    rol: "Directora de Arte / Diseñadora",
    tagline: "Dirección de Arte",
    accent: "#f1e3bc",
    portrait: "imgs/palo-portrait.svg",
    email: "hola@boleadora.studio",
    social: {
      behance: "#",
      linkedin: "#",
      dribbble: "#",
      mail: "mailto:hola@boleadora.studio",
    },
    bio: [
      "Directora de arte y diseñadora. Construyo universos visuales para marcas, campañas y piezas de contenido, del concepto al arte final.",
      "Me apasiona la tipografía, el color y encontrar el tono justo para cada historia. Cuido cada detalle para que la idea llegue clara y con personalidad.",
      "Hablemos de tu próximo proyecto: hola@boleadora.studio.",
    ],
    trayectoria: [
      { puesto: "Directora de Arte, Boleadora Intergaláctica.", periodo: "(2023 - HOY)" },
      { puesto: "Diseñadora Gráfica Sr, Agencia.", periodo: "(2020 - 2023)" },
      { puesto: "Diseñadora Ssr, Estudio de Branding.", periodo: "(2018 - 2020)" },
      { puesto: "Freelancer.", periodo: "(2017 - 2018)" },
    ],
    estudios: {
      izquierda: [
        { texto: "Licenciatura en Diseño Gráfico.", periodo: "(2017)" },
        { texto: "Dirección de Arte Publicitaria.", periodo: "(2019)" },
      ],
      derecha: [
        { texto: "Branding & Identidad Visual.", periodo: "(2021)" },
        { texto: "Tipografía Editorial, Taller.", periodo: "(2020)" },
        { texto: "Ilustración Digital.", periodo: "(2019)" },
      ],
    },
    herramientas: [
      "Illustrator", "Photoshop", "InDesign", "Figma", "After Effects",
      "Procreate",
    ],
    idiomas: ["Español nativo", "Inglés avanzado"],
  },

  {
    id: "ceci",
    nombre: "Ceci",
    nombreCompleto: "Ceci",
    saludo: "Hola, soy Ceci",
    rol: "Compositing / VFX & Motion",
    tagline: "VFX & Compositing",
    accent: "#d8c8f1",
    portrait: "imgs/ceci-portrait.svg",
    email: "hola@boleadora.studio",
    social: {
      behance: "#",
      linkedin: "#",
      dribbble: "#",
      mail: "mailto:hola@boleadora.studio",
    },
    bio: [
      "Artista de compositing y VFX. Integro tomas, genero efectos y le doy vida al motion graphics para que cada plano se sienta real y pulido.",
      "Disfruto resolver desafíos técnicos y visuales, del keying y el rotoscopiado al 3D y la composición final.",
      "¿Sumamos magia a tu pieza? Escribíme a hola@boleadora.studio.",
    ],
    trayectoria: [
      { puesto: "Compositing / VFX Artist, Boleadora Intergaláctica.", periodo: "(2023 - HOY)" },
      { puesto: "Motion & VFX, Freelance.", periodo: "(2021 - 2023)" },
      { puesto: "Compositing Ssr, Postproductora.", periodo: "(2019 - 2021)" },
      { puesto: "Asistente de Postproducción.", periodo: "(2018 - 2019)" },
    ],
    estudios: {
      izquierda: [
        { texto: "Técnicatura en Cine de Animación.", periodo: "(2018)" },
        { texto: "VFX & Compositing.", periodo: "(2020)" },
      ],
      derecha: [
        { texto: "Nuke Compositing, Curso.", periodo: "(2022)" },
        { texto: "Cinema 4D & 3D Motion.", periodo: "(2021)" },
        { texto: "Motion Design, After Effects.", periodo: "(2020)" },
      ],
    },
    herramientas: [
      "After Effects", "Nuke", "Cinema 4D", "Photoshop", "Premiere",
      "DaVinci Resolve",
    ],
    idiomas: ["Español nativo", "Inglés intermedio"],
  },
];

function getMember(id) {
  return TEAM.find((m) => m.id === id) || null;
}
